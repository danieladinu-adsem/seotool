...
async function loadProjects(userId) {
  ...
  try {
    ...
    const result = await Promise.all(projects.map(async project => {
      ...
      return { ...project, auto_check: project.auto_check !== false, location_code: project.location_code||defCountry.location_code, se_domain: project.se_domain||defCountry.se_domain, country: project.country||defCountry.name, keywords };
    }));
...
async function saveProject(project, userId) {
  ...
  try {
    const { error } = await supabase
      .from('projects')
      .upsert({ id: String(project.id), name: project.name, url: project.url, user_id: userId, auto_check: project.auto_check !== false, location_code: project.location_code||2642, se_domain: project.se_domain||'google.ro', country: project.country||'România' });
    if (error) {
...
async function saveProjects(projects, userId) {
  ...
  try {
    for (const project of projects || []) {
      // 1. Upsert project
      const projRow = { id: String(project.id), name: project.name, url: project.url || '', user_id: userId, auto_check: project.auto_check !== false, location_code: project.location_code||2642, se_domain: project.se_domain||'google.ro', country: project.country||'România' };
