...
const COUNTRIES = [
  ...
  {name:"Turcia",     location_code:2792, se_domain:"google.com.tr", language_code:"tr"},
  {name:"UK",         location_code:2826, se_domain:"google.co.uk",  language_code:"en"},
  {name:"SUA",        location_code:2840, se_domain:"google.com",    language_code:"en"},
  {name:"Germania",   location_code:2276, se_domain:"google.de",     language_code:"de"},
  {name:"Franța",     location_code:2250, se_domain:"google.fr",     language_code:"fr"},
  {name:"Italia",     location_code:2380, se_domain:"google.it",     language_code:"it"},
  {name:"Spania",     location_code:2724, se_domain:"google.es",     language_code:"es"},
  {name:"Polonia",    location_code:2616, se_domain:"google.pl",     language_code:"pl"},
  {name:"Ungaria",    location_code:2348, se_domain:"google.hu",     language_code:"hu"},
  {name:"Bulgaria",   location_code:2100, se_domain:"google.bg",     language_code:"bg"},
  {name:"Grecia",     location_code:2300, se_domain:"google.gr",     language_code:"el"},
  {name:"Portugalia", location_code:2620, se_domain:"google.pt",     language_code:"pt"},
  {name:"Olanda",     location_code:2528, se_domain:"google.nl",     language_code:"nl"},
  {name:"Belgia",     location_code:2056, se_domain:"google.be",     language_code:"nl"},
  {name:"Austria",    location_code:2040, se_domain:"google.at",     language_code:"de"},
  {name:"Elveția",    location_code:2756, se_domain:"google.ch",     language_code:"de"},
  {name:"Cehia",      location_code:2203, se_domain:"google.cz",     language_code:"cs"},
  {name:"Suedia",     location_code:2752, se_domain:"google.se",     language_code:"sv"},
  {name:"Norvegia",   location_code:2578, se_domain:"google.no",     language_code:"no"},
  {name:"Danemarca",  location_code:2208, se_domain:"google.dk",     language_code:"da"},
  {name:"Finlanda",   location_code:2246, se_domain:"google.fi",     language_code:"fi"},
];
...
async function generateKeywords(query, location_code=2642, language_code='ro') {
  try {
    const response = await fetch('/api/keywords', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ keywords: [query], location_code, language_code }),
    });
...
async function loadProjects(userId) {
  ...
  try {
    ...
    const { data: projectsData, error: projError } = await supabase
      .from('projects')
      .select('id, name, url, user_id, auto_check, location_code, se_domain, country, sort_order, monthly_report, monthly_report_day')
      .eq('user_id', userId)
    ...
    const result = await Promise.all(projects.map(async project => {
      ...
      const keywords = await Promise.all((kwData || []).map(async kw => {
        ...
        return { ...kw, url: cleanUrl(kw.url), history: histData || [] };
      }));

      const defCountry=COUNTRIES[0];
      return { ...project, auto_check: project.auto_check !== false, monthly_report: project.monthly_report === true, monthly_report_day: project.monthly_report_day || 1, location_code: project.location_code||defCountry.location_code, se_domain: project.se_domain||defCountry.se_domain, country: project.country||defCountry.name, keywords };
    }));
...
async function saveProject(project, userId) {
  ...
  try {
    const { error } = await supabase
      .from('projects')
      .upsert({ id: String(project.id), name: project.name, url: project.url, user_id: userId, auto_check: project.auto_check !== false, monthly_report: project.monthly_report === true, monthly_report_day: project.monthly_report_day || 1, location_code: project.location_code||2642, se_domain: project.se_domain||'google.ro', country: project.country||'România' });
    if (error) {
  ...
  try {
    for (const project of projects || []) {
      ...
      for (const kw of project.keywords || []) {
        ...
        // 3. Salvează keyword_history: upsert (nu duplică per keyword+date)
        ...
        if (historyRows.length > 0) {
          console.log('[saveProjects] upsert history', historyRows.length, 'rows pentru', kw.keyword);
          const { error: histError } = await supabase
            .from('keyword_history')
            .upsert(historyRows, { onConflict: 'keyword_id,date' });
          if (histError) {
            console.log('Supabase error details:', JSON.stringify(histError));
            console.error('[saveProjects] eroare upsert history kw', kw.id, histError.message);
          }
        }
      }
    }
...
function PositionChart({history,externalRange}){const[tip,setTip]=useState(null);const[range,setRange]=useState("14");const activeRange=externalRange||range;if(!history||history.length<1)return null;const allSliced=activeRange==="all"?history:history.slice(-parseInt(activeRange));const filtered=allSliced.filter(e=>e.position!=null);const nullData=allSliced.filter(e=>e.position==null);if(filtered.length<1&&nullData.length<1)return null;const W=220,H=50,padL=4,padR=24,padY=4;if(filtered.length===1&&nullData.length===0){const e=filtered[0];return <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:11,color:C.navy,fontWeight:600}}>#{e.position}</span><span style={{fontSize:10,color:C.grayText}}>{e.date}</span></div>;}if(filtered.length===0){const last=nullData[nullData.length-1];return <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:10,color:C.grayText,fontStyle:"italic"}}>negăsit în top 100</span><span style={{fontSize:10,color:C.grayText}}>{last.date}</span></div>;}const maxP=Math.max(...filtered.map(e=>e.position)),minP=Math.min(...filtered.map(e=>e.position));const tsOf=d=>new Date(d+"T00:00:00").getTime();const allSorted=[...allSliced].sort((a,b)=>a.date>b.date?1:-1);const minTs=tsOf(allSorted[0].date),maxTs=tsOf(allSorted[allSorted.length-1].date),tsRange=maxTs-minTs||1;const toX=d=>padL+((tsOf(d)-minTs)/tsRange)*(W-padL-padR);const pts=filtered.map(e=>({x:toX(e.date),y:padY+((e.position-minP)/((maxP-minP)||1))*(H-padY*2),pos:e.position,date:e.date}));const nullPts=nullData.map(e=>({x:toX(e.date),y:H-3,pos:null,date:e.date}));const allPts=[...pts,...nullPts].sort((a,b)=>a.date>b.date?1:-1);const poly=pts.length>1?pts.map(p=>`${p.x},${p.y}`).join(" "):"";return <div style={{display:"flex",flexDirection:"column",gap:4}}>{!externalRange&&<div style={{display:"flex",gap:4}}>{[["14","14z"],["all","Tot"]].map(([val,label])=><button key={val} onClick={()=>setRange(val)} style={{padding:"2px 7px",fontSize:10,borderRadius:5,border:"1.5px solid",cursor:"pointer",fontWeight:500,borderColor:activeRange===val?C.orange:C.border,background:activeRange===val?C.orangeLight:C.white,color:activeRange===val?C.orange:C.grayText}}>{label}</button>)}</div>}<div style={{position:"relative",display:"inline-block"}} onMouseLeave={()=>setTip(null)}><svg width={W} height={H} onMouseMove={e=>{const r=e.currentTarget.getBoundingClientRect();const mx=e.clientX-r.left;setTip(allPts.reduce((a,b)=>Math.abs(b.x-mx)<Math.abs(a.x-mx)?b:a));}}><rect x={0} y={0} width={W} height={H} fill="transparent"/>{poly&&<polyline points={poly} fill="none" stroke={C.navy} strokeWidth="2" strokeLinejoin="round"/>}{pts.length===1&&<circle cx={pts[0].x} cy={pts[0].y} r={3} fill={C.navy}/>}{nullPts.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r={3} fill={C.grayText} opacity={0.6}/>)}{tip&&<><line x1={tip.x} y1={0} x2={tip.x} y2={H} stroke={C.navy} strokeWidth="1" strokeDasharray="3,2"/><circle cx={tip.x} cy={tip.y} r={3} fill={tip.pos!=null?C.navy:C.grayText}/></>}</svg>{tip&&<div style={{position:"absolute",bottom:"calc(100% + 4px)",left:tip.x>(W-60)?"auto":tip.x-20,right:tip.x>(W-60)?0:"auto",background:C.navy,color:C.white,fontSize:11,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",zIndex:10}}>{tip.pos!=null?`#${tip.pos} - ${tip.date.split('-').reverse().join('.')}`:`negăsit în top 100 - ${tip.date.split('-').reverse().join('.')}`}</div>}</div></div>;}
...
function RankTracker({pendingKeywords,onPendingConsumed,onProjectsLoaded,initialProjectId,userId}){
  ...
  const[projects,setProjects]=useState(null);const[loading,setLoading]=useState(true);const[activeProject,setActiveProject]=useState(initialProjectId||null);const[showNewProject,setShowNewProject]=useState(false);const[newProjectName,setNewProjectName]=useState("");const[newProjectUrl,setNewProjectUrl]=useState("");const[newProjectCountry,setNewProjectCountry]=useState("România");const[newKw,setNewKw]=useState("");const[newDevice,setNewDevice]=useState("desktop");const[activeDevice,setActiveDevice]=useState("desktop");const[chartRange,setChartRange]=useState("14");const[showAddKw,setShowAddKw]=useState(false);const[showBulkAdd,setShowBulkAdd]=useState(false);const[bulkText,setBulkText]=useState("");const[sortKw,setSortKw]=useState("volume_desc");const[checking,setChecking]=useState(false);const[checkProgress,setCheckProgress]=useState({done:0,total:0});const[checkingKwIds,setCheckingKwIds]=useState(new Set());const[updatingVolumes,setUpdatingVolumes]=useState(false);const[showDeleteConfirm,setShowDeleteConfirm]=useState(false);const[deleting,setDeleting]=useState(false);const[editingPosKwId,setEditingPosKwId]=useState(null);const[editingPosVal,setEditingPosVal]=useState("");
  ...
  const addProject=()=>{if(!newProjectName.trim())return;const country=COUNTRIES.find(c=>c.name===newProjectCountry)||COUNTRIES[0];const proj={id:Date.now(),name:newProjectName.trim(),url:newProjectUrl.trim(),location_code:country.location_code,se_domain:country.se_domain,country:country.name,keywords:[]};const updated=[...(projects||[]),proj];saveAndUpdate(updated);setActiveProject(proj.id);setShowNewProject(false);setNewProjectName("");setNewProjectUrl("");setNewProjectCountry("România");};
  const fetchAndSaveVolumes=async(projId,kwList)=>{
    if(!kwList?.length)return;
    try{
      const proj=(projects||[]).find(p=>p.id===projId);
      const country=COUNTRIES.find(c=>c.location_code===proj?.location_code)||COUNTRIES[0];
      const res=await fetch('/api/keyword-volumes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({keywords:kwList.map(k=>k.keyword),location_code:country.location_code,language_code:country.language_code})});
  ...
  const checkSingleKeyword=async(projId,kw)=>{
    ...
    try{
      ...
      const rankUrl=cleanUrl(data.url||kw.url||'');
      const posField=activeDevice==='mobile'?'position_mobile':'position_desktop';
      const newOverrides={...(kw.report_overrides||{})};
      if(newPos&&newPos>0&&(newOverrides.best==null||newPos<newOverrides.best))newOverrides.best=newPos;
      if(supabase){
        const{error:kwErr}=await supabase.from('keywords').update({[posField]:newPos,position:newPos??kw.position,url:rankUrl,report_overrides:newOverrides}).eq('id',String(kw.id));
        ...
        if(histErr)console.error('[history upsert error]',kw.keyword,JSON.stringify(histErr));else console.log('[history saved]',kw.keyword,today,'pos:',newPos);
      }
      const prevHistory=(kw.history||[]).filter(h=>h.date!==today).slice(-29);
      const history=[...prevHistory,{date:today,position:newPos}];
      const updated=(projects||[]).map(p=>p.id!==projId?p:{...p,keywords:p.keywords.map(k=>k.id!==kw.id?k:{...k,[posField]:newPos,position:newPos??kw.position,url:rankUrl,history,report_overrides:newOverrides})});
      setProjects(updated);
      onProjectsLoaded&&onProjectsLoaded(updated);
  ...
  const saveManualPosition=async(projId,kw,posVal)=>{
    const pos=parseInt(posVal);
    if(!posVal||isNaN(pos)||pos<1)return;
    const today=new Date().toISOString().split('T')[0];
    const posField=activeDevice==='mobile'?'position_mobile':'position_desktop';
    const newOverrides={...(kw.report_overrides||{})};
    if(newOverrides.best==null||pos<newOverrides.best)newOverrides.best=pos;
    if(supabase){
      await supabase.from('keywords').update({[posField]:pos,position:pos,position_override:pos,report_overrides:newOverrides}).eq('id',String(kw.id));
      await supabase.from('keyword_history').upsert({keyword_id:String(kw.id),position:pos,date:today},{onConflict:'keyword_id,date'});
    }
    const prevHistory=(kw.history||[]).filter(h=>h.date!==today).slice(-29);
    const history=[...prevHistory,{date:today,position:pos}];
    const updated=(projects||[]).map(p=>p.id!==projId?p:{...p,keywords:p.keywords.map(k=>k.id!==kw.id?k:{...k,[posField]:pos,position:pos,position_override:pos,history,report_overrides:newOverrides})});
    setProjects(updated);
    onProjectsLoaded&&onProjectsLoaded(updated);
    setEditingPosKwId(null);setEditingPosVal("");
  };
  const [editingInitKwId,setEditingInitKwId]=useState(null);
  const [editingInitVal,setEditingInitVal]=useState("");
  const [savedInitKwId,setSavedInitKwId]=useState(null);
  const [editingBestKwId,setEditingBestKwId]=useState(null);
  const [editingBestVal,setEditingBestVal]=useState("");
  const [savedBestKwId,setSavedBestKwId]=useState(null);
  const saveInitialPosition=async(projId,kw,val)=>{
    const pos=parseInt(val);
    if(!val||isNaN(pos)||pos<1){setEditingInitKwId(null);setEditingInitVal("");return;}
    const newOverrides={...(kw.report_overrides||{})};
    if(newOverrides.best==null||pos<newOverrides.best)newOverrides.best=pos;
    if(supabase){
      const{error}=await supabase.from('keywords').update({initial_position:pos,report_overrides:newOverrides}).eq('id',String(kw.id));
      if(error)console.error('[saveInitialPosition]',kw.keyword,JSON.stringify(error));
    }
    const updated=(projects||[]).map(p=>p.id!==projId?p:{...p,keywords:p.keywords.map(k=>k.id!==kw.id?k:{...k,initial_position:pos,report_overrides:newOverrides})});
    setProjects(updated);
    onProjectsLoaded&&onProjectsLoaded(updated);
    setEditingInitKwId(null);setEditingInitVal("");
    setSavedInitKwId(kw.id);setTimeout(()=>setSavedInitKwId(null),1500);
  };
  const saveBestPosition=async(projId,kw,val)=>{
    const pos=val===''?null:parseInt(val);
    if(val!==''&&(isNaN(pos)||pos<1)){setEditingBestKwId(null);setEditingBestVal("");return;}
    const overrides={...(kw.report_overrides||{})};
    if(pos===null)delete overrides.best;else overrides.best=pos;
    if(supabase){
      const{error}=await supabase.from('keywords').update({report_overrides:overrides}).eq('id',String(kw.id));
      if(error)console.error('[saveBestPosition]',kw.keyword,JSON.stringify(error));
    }
    const updated=(projects||[]).map(p=>p.id!==projId?p:{...p,keywords:p.keywords.map(k=>k.id!==kw.id?k:{...k,report_overrides:overrides})});
    setProjects(updated);
    onProjectsLoaded&&onProjectsLoaded(updated);
    setEditingBestKwId(null);setEditingBestVal("");
    setSavedBestKwId(kw.id);setTimeout(()=>setSavedBestKwId(null),1500);
  };
    ...
    <div>
      ...
      {projects?.length>0&&<div style={{display:"flex",gap:20,flexDirection:isMobile?"column":"row"}}>
        ...
        {proj&&<div style={{flex:1,minWidth:0}}>
          <div style={{marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}><div><div style={{fontWeight:700,fontSize:16,color:C.navy}}>{proj.name}</div><div style={{display:"flex",gap:10,marginTop:2,flexWrap:"wrap"}}>{proj.url&&<span style={{fontSize:12,color:C.orange}}>{proj.url}</span>}{proj.se_domain&&<span style={{fontSize:12,color:C.grayText}}>🌍 {proj.se_domain}</span>}</div></div><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
<button onClick={async()=>{const newVal=proj.auto_check!==false?false:true;const updated=(projects||[]).map(p=>p.id!==proj.id?p:{...p,auto_check:newVal});setProjects(updated);onProjectsLoaded&&onProjectsLoaded(updated);if(supabase){const{error}=await supabase.from('projects').update({auto_check:newVal}).eq('id',String(proj.id));if(error)console.error('[auto_check] save error:',JSON.stringify(error));}}} title="Activează/dezactivează verificarea automată zilnică" style={{display:"flex",alignItems:"center",gap:7,padding:"6px 12px",borderRadius:8,border:`1.5px solid ${proj.auto_check!==false?C.orange:C.border}`,background:proj.auto_check!==false?C.orangeLight:C.gray,cursor:"pointer",fontSize:12,fontWeight:600,color:proj.auto_check!==false?C.orange:C.grayText,whiteSpace:"nowrap"}}><span style={{width:28,height:16,borderRadius:8,background:proj.auto_check!==false?"#f97316":"#d1d5db",display:"inline-block",position:"relative",transition:"background 0.2s",flexShrink:0}}><span style={{position:"absolute",top:2,left:proj.auto_check!==false?14:2,width:12,height:12,borderRadius:"50%",background:C.white,transition:"left 0.2s"}}/></span>Verificare automată zilnică</button>
<button onClick={async()=>{const newVal=proj.monthly_report!==true;const updated=(projects||[]).map(p=>p.id!==proj.id?p:{...p,monthly_report:newVal});setProjects(updated);onProjectsLoaded&&onProjectsLoaded(updated);if(supabase){const{error}=await supabase.from('projects').update({monthly_report:newVal}).eq('id',String(proj.id));if(error)console.error('[monthly_report] save error:',JSON.stringify(error));}}} title="Activează/dezactivează raportarea lunară" style={{display:"flex",alignItems:"center",gap:7,padding:"6px 12px",borderRadius:8,border:`1.5px solid ${proj.monthly_report===true?"#7c3aed":C.border}`,background:proj.monthly_report===true?"#f5f3ff":C.gray,cursor:"pointer",fontSize:12,fontWeight:600,color:proj.monthly_report===true?"#7c3aed":C.grayText,whiteSpace:"nowrap"}}><span style={{width:28,height:16,borderRadius:8,background:proj.monthly_report===true?"#7c3aed":"#d1d5db",display:"inline-block",position:"relative",transition:"background 0.2s",flexShrink:0}}><span style={{position:"absolute",top:2,left:proj.monthly_report===true?14:2,width:12,height:12,borderRadius:"50%",background:C.white,transition:"left 0.2s"}}/></span>Raportare lunară</button>
{proj.monthly_report===true&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 10px",borderRadius:8,border:"1.5px solid #7c3aed",background:"#f5f3ff"}}><span style={{fontSize:12,color:"#7c3aed",fontWeight:600,whiteSpace:"nowrap"}}>Ziua</span><input type="number" min={1} max={28} value={proj.monthly_report_day||1} onChange={async e=>{const day=Math.min(28,Math.max(1,parseInt(e.target.value)||1));const updated=(projects||[]).map(p=>p.id!==proj.id?p:{...p,monthly_report_day:day});setProjects(updated);onProjectsLoaded&&onProjectsLoaded(updated);if(supabase){await supabase.from('projects').update({monthly_report_day:day}).eq('id',String(proj.id));}}} style={{width:48,padding:"3px 6px",border:"1.5px solid #7c3aed",borderRadius:6,fontSize:13,fontWeight:700,textAlign:"center",outline:"none",background:C.white,color:"#7c3aed"}}/><span style={{fontSize:12,color:"#7c3aed",fontWeight:600,whiteSpace:"nowrap"}}>din lună</span></div>}
</div></div>
          ...
          :<div>
            <ProjectChart keywords={proj.keywords} activeDevice={activeDevice} chartRange={chartRange} onRangeChange={setChartRange}/>
            ...
            <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,overflow:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}><thead><tr style={{background:C.gray,borderBottom:`1px solid ${C.border}`}}><th style={{padding:"12px 16px",textAlign:"left",fontSize:12,fontWeight:600,color:C.grayText,textTransform:"uppercase",letterSpacing:"0.05em"}}>Keyword</th><th style={{padding:"12px 16px",textAlign:"center",fontSize:12,fontWeight:600,color:C.grayText,textTransform:"uppercase",letterSpacing:"0.05em",width:120}}>Pozitie</th><th style={{padding:"12px 16px",textAlign:"center",fontSize:12,fontWeight:600,color:C.grayText,textTransform:"uppercase",letterSpacing:"0.05em",width:80}}>Volum</th><th style={{padding:"12px 16px",textAlign:"left",fontSize:12,fontWeight:600,color:C.grayText,textTransform:"uppercase",letterSpacing:"0.05em"}}>URL</th><th style={{padding:"12px 16px",textAlign:"center",fontSize:12,fontWeight:600,color:C.grayText,textTransform:"uppercase",letterSpacing:"0.05em",width:100}}>Poz. inițială</th><th style={{padding:"12px 16px",textAlign:"center",fontSize:12,fontWeight:600,color:C.green,textTransform:"uppercase",letterSpacing:"0.05em",width:90}}>Best</th><th style={{padding:"12px 16px",textAlign:"left",fontSize:12,fontWeight:600,color:C.grayText,textTransform:"uppercase",letterSpacing:"0.05em",width:220}}>Evolutie</th><th style={{padding:"12px 16px",width:40}}></th></tr></thead><tbody>{getSortedKeywords(proj.keywords).map(kw=><tr key={kw.id} style={{borderBottom:`1px solid ${C.grayMid}`}} onMouseEnter={e=>e.currentTarget.style.background=C.gray} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><td style={{padding:"13px 16px",fontWeight:500,fontSize:14,color:C.navy}}>{kw.keyword}</td><td style={{padding:"13px 16px",textAlign:"center"}}>{editingPosKwId===kw.id?<div style={{display:"flex",gap:4,alignItems:"center",justifyContent:"center"}}><input autoFocus value={editingPosVal} onChange={e=>setEditingPosVal(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")saveManualPosition(proj.id,kw,editingPosVal);if(e.key==="Escape"){setEditingPosKwId(null);setEditingPosVal("");}}} placeholder="#" style={{width:52,padding:"4px 8px",border:`1.5px solid ${C.orange}`,borderRadius:6,fontSize:13,textAlign:"center",outline:"none"}}/><button onClick={()=>saveManualPosition(proj.id,kw,editingPosVal)} style={{background:C.orange,color:C.white,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:12}}>✓</button><button onClick={()=>{setEditingPosKwId(null);setEditingPosVal("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:C.grayText}}>✕</button></div>:(()=>{const hist=kw.history&&kw.history.length>0?[...kw.history].sort((a,b)=>a.date>b.date?1:-1):[];const lastPos=hist.length>0?([...hist].reverse().find(h=>h.position!=null)?.position??null):null;const latestEntry=hist.length>0?hist[hist.length-1]:null;const lastCheckedNull=!!(latestEntry&&latestEntry.position==null);const pos=lastPos!=null?lastPos:(activeDevice==='mobile'?kw.position_mobile:kw.position_desktop);return <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><div style={{cursor:"pointer"}} title="Click pentru a introduce manual poziția" onClick={()=>{setEditingPosKwId(kw.id);setEditingPosVal(lastCheckedNull?"":String(pos||""));}}>{lastCheckedNull?<span style={{fontWeight:700,fontSize:15,color:C.grayText,background:C.gray,padding:"4px 10px",borderRadius:8,minWidth:36,textAlign:"center",display:"inline-block"}}>&gt;100</span>:<PositionBadge pos={pos}/>}</div><button onClick={()=>checkSingleKeyword(proj.id,kw)} disabled={checkingKwIds.has(kw.id)} title="Verifică poziția acum" style={{background:"none",border:"none",cursor:checkingKwIds.has(kw.id)?"default":"pointer",fontSize:15,padding:2,opacity:checkingKwIds.has(kw.id)?0.5:1,lineHeight:1}}>{checkingKwIds.has(kw.id)?"⏳":"🔄"}</button></div>;})()}</td><td style={{padding:"13px 16px",textAlign:"center",fontSize:13,fontWeight:600,color:C.grayText}}>{kw.volume>0?fmtN(kw.volume):"—"}</td><td style={{padding:"13px 16px"}}><input value={cleanUrl(kw.url)||''} onChange={e=>updateUrl(proj.id,kw.id,e.target.value)} placeholder="https://..." style={{width:"100%",padding:"5px 8px",border:`1.5px solid ${C.border}`,borderRadius:6,fontSize:12,outline:"none",boxSizing:"border-box",color:C.orange}} onFocus={e=>e.target.style.borderColor=C.orange} onBlur={e=>e.target.style.borderColor=C.border}/></td><td style={{padding:"13px 16px",textAlign:"center"}}>{(()=>{const hist=[...(kw.history||[])].sort((a,b)=>a.date>b.date?1:-1);const firstPos=hist.find(h=>h.position!=null)?.position??null;const initPos=kw.initial_position??firstPos??null;if(editingInitKwId===kw.id)return <div style={{display:"flex",gap:4,alignItems:"center",justifyContent:"center"}}><input autoFocus value={editingInitVal} onChange={e=>setEditingInitVal(e.target.value)} onBlur={()=>saveInitialPosition(proj.id,kw,editingInitVal)} onKeyDown={e=>{if(e.key==="Enter")saveInitialPosition(proj.id,kw,editingInitVal);if(e.key==="Escape"){setEditingInitKwId(null);setEditingInitVal("");}}} placeholder="#" style={{width:52,padding:"4px 8px",border:`1.5px solid ${C.orange}`,borderRadius:6,fontSize:13,textAlign:"center",outline:"none"}}/><button onMouseDown={e=>{e.preventDefault();saveInitialPosition(proj.id,kw,editingInitVal);}} style={{background:C.orange,color:C.white,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:12}}>✓</button><button onMouseDown={e=>{e.preventDefault();setEditingInitKwId(null);setEditingInitVal("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:C.grayText}}>✕</button></div>;if(initPos)return <div style={{cursor:"pointer",borderRadius:8,border:`2px solid ${savedInitKwId===kw.id?C.green:'transparent'}`,transition:"border-color 0.3s"}} title="Click pentru a edita" onClick={()=>{setEditingInitKwId(kw.id);setEditingInitVal(String(initPos));}}><PositionBadge pos={initPos}/></div>;return <div style={{cursor:"pointer",fontSize:12,color:C.grayText}} title="Click pentru a introduce manual" onClick={()=>{setEditingInitKwId(kw.id);setEditingInitVal("");}}>✏️ adaugă</div>;})()}</td><td style={{padding:"13px 16px",textAlign:"center"}}>{(()=>{const curPos=activeDevice==='mobile'?kw.position_mobile:kw.position_desktop;const allPos=[curPos,kw.initial_position,...(kw.history||[]).map(h=>h.position)].filter(p=>p!=null&&p>0);const bestAuto=allPos.length?Math.min(...allPos):null;const savedBest=kw.report_overrides?.best;const bestVal=savedBest??bestAuto;if(editingBestKwId===kw.id)return <div style={{display:"flex",gap:4,alignItems:"center",justifyContent:"center"}}><input autoFocus value={editingBestVal} onChange={e=>setEditingBestVal(e.target.value)} onBlur={()=>saveBestPosition(proj.id,kw,editingBestVal)} onKeyDown={e=>{if(e.key==="Enter")saveBestPosition(proj.id,kw,editingBestVal);if(e.key==="Escape"){setEditingBestKwId(null);setEditingBestVal("");}}} placeholder="#" style={{width:52,padding:"4px 8px",border:`1.5px solid ${C.green}`,borderRadius:6,fontSize:13,textAlign:"center",outline:"none"}}/><button onMouseDown={e=>{e.preventDefault();saveBestPosition(proj.id,kw,editingBestVal);}} style={{background:C.green,color:C.white,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:12}}>✓</button><button onMouseDown={e=>{e.preventDefault();setEditingBestKwId(null);setEditingBestVal("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:C.grayText}}>✕</button></div>;if(bestVal)return <div style={{cursor:"pointer",borderRadius:8,border:`2px solid ${savedBestKwId===kw.id?C.green:'transparent'}`,transition:"border-color 0.3s"}} title="Click pentru a edita" onClick={()=>{setEditingBestKwId(kw.id);setEditingBestVal(String(bestVal));}}><span style={{fontWeight:700,fontSize:15,color:C.green,background:C.greenLight,padding:"4px 10px",borderRadius:8,minWidth:36,textAlign:"center",display:"inline-block"}}>#{bestVal}</span></div>;return <div style={{cursor:"pointer",fontSize:12,color:C.grayText}} title="Click pentru a introduce manual" onClick={()=>{setEditingBestKwId(kw.id);setEditingBestVal("");}}>✏️ adaugă</div>;})()}</td><td style={{padding:"13px 16px"}}><PositionChart history={kw.history} externalRange={chartRange}/></td><td style={{padding:"13px 16px"}}><button onClick={()=>removeKeyword(proj.id,kw.id)} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,color:"#fca5a5",padding:4}}>×</button></td></tr>)}</tbody></table></div>
...
function ReportPreview({ config, project, p1Label, p2Label, onKeywordUpdate }) {
  ...
  const getVal = (kw, field, fallback) => {
    if (field === 'initial_position') return kw.initial_position ?? fallback;
  ...
  const movers = kws.map((k,i)=>({
    ...
    curPos:posNow[i],
    prevPos:posPrev[i],
    prevPosDesktop:(()=>{const hist=[...(k.history||[])].sort((a,b)=>a.date>b.date?1:-1);const firstPos=hist.find(h=>h.position!=null)?.position??null;return k.initial_position??firstPos??null;})(),
                ...
                {[
                  ...
                  <div key={i} style={{background:C.gray,borderRadius:10,padding:"14px 16px"}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      ...
                      <span style={{fontSize:28,fontWeight:800,color:accentColor}}>{s.desk}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{borderRadius:10,border:`1px solid ${C.border}`,overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:750}}>
                  ...
                  <tbody>{[...movers].sort((a,b)=>(b.volume||0)-(a.volume||0)).slice(0,config.maxKeywords||999).map((kw,i)=>{
                    ...
                    const autoBest=allPos.length?Math.min(...allPos):null;
                    const best=savedBest??autoBest;
                    return(
                      <tr key={i} style={{borderTop:`1px solid ${C.grayMid}`}}>
                        ...
                        <td style={{padding:"10px 14px"}}><EditableCell kwId={kw.id} field="position_desktop" fallback={kw.position_desktop} render={v=>{if(!v)return <span style={{color:C.grayMid,fontSize:12}}>—</span>;const prev=kw.report_overrides?.poz_anterioara??getPosAroundFirst(kw);const imp=prev&&v<prev;return <span style={{display:"inline-flex",alignItems:"center",gap:4}}><PositionBadge pos={v}/>{imp&&<span style={{color:C.green,fontSize:20,fontWeight:900,lineHeight:1,display:"inline-flex",alignItems:"center"}}>↑</span>}</span>;}}/></td>
      ...
      {step==="config" && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:20}}>
              <div style={{fontWeight:700,fontSize:14,marginBottom:14,color:C.navy}}>📊 Coloane tabel keywords</div>
              <div style={{display:"flex",alignItems:"center",gap:10,background:showTrend?`${accentColor}0a`:C.gray,border:`1.5px solid ${showTrend?accentColor:C.border}`,borderRadius:9,padding:"10px 12px"}}>
...
export default function App() {
  ...
  const handleAddToTracker = (keywords, projectId) => { setPendingKeywords(keywords); if(projectId&&!String(projectId).startsWith('new:')){setSelectedProjectId(projectId);}else if(String(projectId).startsWith('new:')){const name=String(projectId).replace('new:','');if(name){const newProj={id:Date.now(),name,url:'',keywords:[],auto_check:true};setTrackerProjects(p=>[...(p||[]),newProj]);setSelectedProjectId(newProj.id);}}setPage("rank"); };
  ...
  const renderPage = () => {
    ...
    if (page==="keywords") return <KeywordResearch onAddToTracker={handleAddToTracker} projects={trackerProjects}/>;
  ...
  return (
    <div style={{display:"flex",minHeight:"112vh",fontFamily:"Inter,'Segoe UI',sans-serif",background:C.gray,color:C.navy}}>
      ...
      {/* Sidebar */}
      {isMobile ? (
        <div style={{position:"fixed",top:0,left:drawerOpen?0:-240,width:240,height:"200vh",background:C.navy,display:"flex",flexDirection:"column",flexShrink:0,zIndex:100,transition:"left 0.25s ease",boxShadow:drawerOpen?"4px 0 24px rgba(0,0,0,0.3)":"none"}}>
          {sidebarContent}
        </div>
      ) : (
