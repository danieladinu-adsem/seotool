...
function PositionChart({history,externalRange}){const[tip,setTip]=useState(null);const[range,setRange]=useState("14");const activeRange=externalRange||range;if(!history||history.length<1)return null;const filtered=activeRange==="all"?history:history.slice(-parseInt(activeRange));if(filtered.length<1)return null;const W=200,H=50,pad=4;if(filtered.length===1){const e=filtered[0];return <div style={{display:"flex",alignItems:"center",gap:6}}><circle/><span style={{fontSize:11,color:C.navy,fontWeight:600}}>#{e.position}</span><span style={{fontSize:10,color:C.grayText}}>{e.date}</span></div>;}const positions=filtered.map(e=>e.position);const maxP=Math.max(...positions),minP=Math.min(...positions);const tsOf=d=>new Date(d+"T00:00:00").getTime();const minTs=tsOf(filtered[0].date),maxTs=tsOf(filtered[filtered.length-1].date),tsRange=maxTs-minTs||1;const pts=filtered.map(e=>({x:pad+((tsOf(e.date)-minTs)/tsRange)*(W-pad*2),y:pad+((e.position-minP)/((maxP-minP)||1))*(H-pad*2),pos:e.position,date:e.date}));const poly=pts.map(p=>`${p.x},${p.y}`).join(" ");return <div style={{display:"flex",flexDirection:"column",gap:4}}>{!externalRange&&<div style={{display:"flex",gap:4}}>{[["14","14z"],["all","Tot"]].map(([val,label])=><button key={val} onClick={()=>setRange(val)} style={{padding:"2px 7px",fontSize:10,borderRadius:5,border:"1.5px solid",cursor:"pointer",fontWeight:500,borderColor:activeRange===val?C.orange:C.border,background:activeRange===val?C.orangeLight:C.white,color:activeRange===val?C.orange:C.grayText}}>{label}</button>)}</div>}<div style={{position:"relative",display:"inline-block"}} onMouseLeave={()=>setTip(null)}><svg width={W} height={H} onMouseMove={e=>{const r=e.currentTarget.getBoundingClientRect();const mx=e.clientX-r.left;setTip(pts.reduce((a,b)=>Math.abs(b.x-mx)<Math.abs(a.x-mx)?b:a));}}><rect x={0} y={0} width={W} height={H} fill="transparent"/><polyline points={poly} fill="none" stroke={C.navy} strokeWidth="2" strokeLinejoin="round"/>{tip&&<><line x1={tip.x} y1={0} x2={tip.x} y2={H} stroke={C.navy} strokeWidth="1" strokeDasharray="3,2"/><circle cx={tip.x} cy={tip.y} r={3} fill={C.navy}/></>}</svg>{tip&&<div style={{position:"absolute",bottom:"calc(100% + 4px)",left:tip.x>140?"auto":tip.x-20,right:tip.x>140?0:"auto",background:C.navy,color:C.white,fontSize:11,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",zIndex:10}}>{tip.date}: #{tip.pos}</div>}</div></div>;}
