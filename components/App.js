...
function PositionChart({history,externalRange}){const[tip,setTip]=useState(null);const[range,setRange]=useState("14");const activeRange=externalRange||range;if(!history||history.length<1)return null;const filtered=activeRange==="all"?history:history.slice(-parseInt(activeRange));if(filtered.length<1)return null;const W=220,H=50,padL=4,padR=24,padY=4;const withPos=filtered.filter(e=>e.position!=null);if(withPos.length===0){return <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.grayMid,display:"inline-block",flexShrink:0}}/><span style={{fontSize:10,color:C.grayText}}>negăsit {filtered[filtered.length-1]?.date}</span></div>;}if(withPos.length===1&&filtered.length===1){const e=withPos[0];return <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:11,color:C.navy,fontWeight:600}}>#{e.position}</span><span style={{fontSize:10,color:C.grayText}}>{e.date}</span></div>;}const maxP=Math.max(...withPos.map(e=>e.position)),minP=Math.min(...withPos.map(e=>e.position));const tsOf=d=>new Date(d+"T00:00:00").getTime();const allTs=filtered.map(e=>tsOf(e.date));const minTs=Math.min(...allTs),maxTs=Math.max(...allTs),tsRange=maxTs-minTs||1;const allPts=filtered.map(e=>({x:padL+((tsOf(e.date)-minTs)/tsRange)*(W-padL-padR),y:e.position!=null?padY+((e.position-minP)/((maxP-minP)||1))*(H-padY*2):null,pos:e.position,date:e.date}));const validPts=allPts.filter(p=>p.y!=null);const poly=validPts.map(p=>`${p.x},${p.y}`).join(" ");return <div style={{display:"flex",flexDirection:"column",gap:4}}>{!externalRange&&<div style={{display:"flex",gap:4}}>{[["14","14z"],["all","Tot"]].map(([val,label])=><button key={val} onClick={()=>setRange(val)} style={{padding:"2px 7px",fontSize:10,borderRadius:5,border:"1.5px solid",cursor:"pointer",fontWeight:500,borderColor:activeRange===val?C.orange:C.border,background:activeRange===val?C.orangeLight:C.white,color:activeRange===val?C.orange:C.grayText}}>{label}</button>)}</div>}<div style={{position:"relative",display:"inline-block"}} onMouseLeave={()=>setTip(null)}><svg width={W} height={H} onMouseMove={e=>{const r=e.currentTarget.getBoundingClientRect();const mx=e.clientX-r.left;setTip(allPts.reduce((a,b)=>Math.abs(b.x-mx)<Math.abs(a.x-mx)?b:a));}}><rect x={0} y={0} width={W} height={H} fill="transparent"/><polyline points={poly} fill="none" stroke={C.navy} strokeWidth="2" strokeLinejoin="round"/>{allPts.filter(p=>p.y==null).map((p,i)=><circle key={i} cx={p.x} cy={H-padY} r={3} fill={C.grayMid} opacity={0.7} title="negăsit"/>)}{tip&&<><line x1={tip.x} y1={0} x2={tip.x} y2={H} stroke={tip.pos!=null?C.navy:C.grayMid} strokeWidth="1" strokeDasharray="3,2"/>{tip.y!=null&&<circle cx={tip.x} cy={tip.y} r={3} fill={C.navy}/>}</>}</svg>{tip&&<div style={{position:"absolute",bottom:"calc(100% + 4px)",left:tip.x>(W-60)?"auto":tip.x-20,right:tip.x>(W-60)?0:"auto",background:tip.pos!=null?C.navy:"#6B7280",color:C.white,fontSize:11,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",zIndex:10}}>{tip.date}: {tip.pos!=null?`#${tip.pos}`:"negăsit"}</div>}</div></div>;}
