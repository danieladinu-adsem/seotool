                  ...
                  <tbody>{[...movers].sort((a,b)=>(b.volume||0)-(a.volume||0)).slice(0,config.maxKeywords||999).map((kw,i)=>{
                    ...
                    return(
                      <tr key={i} style={{borderTop:`1px solid ${C.grayMid}`}}>
                        ...
                        <td style={{padding:"10px 14px"}}><EditableCell kwId={kw.id} field="position_desktop" fallback={kw.position_desktop} render={v=>{if(!v)return <span style={{color:C.grayMid,fontSize:12}}>—</span>;const prev=getPosAroundFirst(kw);const imp=prev&&v<prev;return <span style={{display:"inline-flex",alignItems:"center",gap:4}}><PositionBadge pos={v}/>{imp&&<span style={{color:C.green,fontSize:20,fontWeight:900,lineHeight:1,display:"inline-flex",alignItems:"center"}}>↑</span>}</span>;}}/></td>
