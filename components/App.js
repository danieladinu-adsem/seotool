...
function ReportPreview({ config, project, p1Label, p2Label, onKeywordUpdate }) {
  ...
  const movers = kws.map((k,i)=>({
    ...
    curPos:posNow[i],
    prevPos:posPrev[i],
    prevPosDesktop:(()=>{const hist=[...(k.history||[])].sort((a,b)=>a.date>b.date?1:-1);return hist.find(h=>h.position!=null)?.position??k.initial_position??null;})(),
